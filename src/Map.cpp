...
bool MapManager::isWall(int cellX, int cellY) {
    ...
    // Añade esta verificación al inicio de tu función isWall en Map.cpp:
    if (activeSecret.isActive && cellX == activeSecret.targetX && cellY == activeSecret.targetY) {
    return true; 
}
}
...
void MapManager::openDoor(int cellX, int cellY) {
    ...
    if (cellX >= 0 && cellX < cur->cols && cellY >= 0 && cellY < cur->rows) {
        if (cur->grid[cellY][cellX].type == 2) cur->grid[cellY][cellX].type = 0; // Se vuelve piso libre
    }
}
...
void MapManager::drawFullMap(sf::RenderWindow& window) {
    ...
    for (int r = 0; r < cur->rows; r++) {
        for (int c = 0; c < cur->cols; c++) {
            ...
            if (cell.type == 1) tile.setFillColor(sf::Color(100, 100, 100));     // Muro
            else if (cell.type == 2) tile.setFillColor(sf::Color(150, 50, 150)); // Puerta
            else if (cell.type == 9) tile.setFillColor(sf::Color(35, 180, 70));   // Salida
            else tile.setFillColor(sf::Color(35, 35, 35));                       // Suelo libre
